<?php
	...
	function generarRecomendacionesArkrit($idUsuario){
		...
		if( !$recomendaciones === false ){
			...
			// 1) Consulto la titulacion en la que estÃ¡ el usuario

			$idTitulacion = consulta( 'id_titulacion', 'alumno', 'id_usuario = ' . $idUsuario );



				// 1.1) Si es de Grado no tiene titulaciones por debajo de Ã©l
			...
			//		titulacion anterior y nivel bÃ¡sico,

			//		titulacion anterior y nivel avanzado,

			//		titulacion actual(igual) y nivel bÃ¡sico y
			...
			for( $i=0; $i<count( $recomendaciones ); $i++ ){
				...
				if( $recomendaciones[$i]['id_titulacion'] <= $idTitulacion ){
					...
					if( $recomendaciones[$i]['id_titulacion'] < $idTitulacion ){
						...
						if( $recomendaciones[$i]['nivel_especializacion'] == 'BÃ¡sico' )
			...
			// 3) Ordenamos todos los arrays segÃºn la posiciÃ³n que tienen en el ranking los libros
			...
			$recomendaciones = array();
				...
				// 4.1) Generamos dos recomendaciones de titulaciones anteriores si no se estÃ¡ en grado
			...
			if( $maxLibrosNivelInferior > count($recomendaciones) )
				...
				// 4.2) Generamos tres o cinco (si se estÃ¡ en grado) recomendaciones de la titulacion actual
	...
	function validarCamposUsuario( $usuario , $comprobarExistente = false, $comprobarContrasena = true, $comprobarBloqueado = false){
		...
		if( $usuario['nombre'] == '' || strlen($usuario['nombre']) > 40 || !preg_match('/^[a-zA-ZñÑÁáÉéÍíÓóÚúÀàÈèÌìÒòÙùÄäËëÏïÖöÜüÂâÊêÎîÔôÛûÇçß\'\s]+$/', $usuario['nombre']) )

			return false;

		if( $usuario['primerApellido'] == '' || strlen($usuario['primerApellido']) > 30 || !preg_match('/^[a-zA-ZñÑÁáÉéÍíÓóÚúÀàÈèÌìÒòÙùÄäËëÏïÖöÜüÂâÊêÎîÔôÛûÇçß\'\s]+$/', $usuario['primerApellido']) )

			return false;

		if( $usuario['segundoApellido'] == '' || strlen($usuario['segundoApellido']) > 30 || !preg_match('/^[a-zA-ZñÑÁáÉéÍíÓóÚúÀàÈèÌìÒòÙùÄäËëÏïÖöÜüÂâÊêÎîÔôÛûÇçß\'\s]+$/', $usuario['segundoApellido']) )
		...
		if( $usuario['nombreUsuario'] == '' || strlen($usuario['nombreUsuario']) > 20 || !preg_match('/^[a-zA-ZñÑÁáÉéÍíÓóÚúÀàÈèÌìÒòÙùÄäËëÏïÖöÜüÂâÊêÎîÔôÛûÇçß\'\s', $usuario['nombreUsuario']) || ($comprobarExistente && existeRegistro('nombre_usuario', $usuario['nombreUsuario'], 'usuario') ) )
	...
	function validarCamposLibro($libro){
		...
		//if( $libro['titulo'] == '' || strlen($libro['titulo']) > 100 || !preg_match('/^[a-zA-Z0-9«¡«±«¿«Ñ«ñ«â«¢«²«À«Ò«ãª¢ª²ªÀªÒªãª¡ª±ª¿ªÑªñªâ«ĞªĞ«äªä_,.&\s]+$/', $libro['titulo']) )
		if( $libro['titulo'] == '' || strlen($libro['titulo']) > 100 || !preg_match('/^[a-zA-Z0-9ñÑÁáÉéÍíÓóÚúÀàÈèÌìÒòÙùÄäËëÏïÖöÜüÂâÊêÎîÔôÛûÇçß_\-.:;\'&\s]+$/', $libro['titulo']) )
			return false;



		if( $libro['tituloOriginal'] == '' || strlen($libro['tituloOriginal']) > 100 || !preg_match('/^[a-zA-Z0-9ñÑÁáÉéÍíÓóÚúÀàÈèÌìÒòÙùÄäËëÏïÖöÜüÂâÊêÎîÔôÛûÇçß_\-.:;\'&\s]+$/', $libro['tituloOriginal']) )

			return false;



		if( $libro['autor'] == '' || strlen($libro['autor']) > 50 || !preg_match('/^[a-zA-Z0-9ñÑÁáÉéÍíÓóÚúÀàÈèÌìÒòÙùÄäËëÏïÖöÜüÂâÊêÎîÔôÛûÇçß_\-.;\'\s]+$/', $libro['autor']) )
	...
	function validarCamposLibroPropuesto($libroPropuesto){
		...
		if(utf8_encode($libroPropuesto['propuestoPara']) !=  utf8_encode('aÃ±adir') && $libroPropuesto['propuestoPara'] != 'eliminar')
	...
	function validarCamposNominacion($nominacion){
		...
		if( $nominacion['titulo'] == '' || strlen($nominacion['titulo']) > 100 || !preg_match('/^[a-zA-Z0-9ñÑÁáÉéÍíÓóÚúÀàÈèÌìÒòÙùÄäËëÏïÖöÜüÂâÊêÎîÔôÛûÇçß_\-.:;\'&\s]+$/', $nominacion['titulo']) )

			return false;



		if( $nominacion['tituloOriginal'] == '' || strlen($nominacion['tituloOriginal']) > 100 || !preg_match('/^[a-zA-Z0-9ñÑÁáÉéÍíÓóÚúÀàÈèÌìÒòÙùÄäËëÏïÖöÜüÂâÊêÎîÔôÛûÇçß_\-.:;\'&\s]+$/', $nominacion['tituloOriginal']) )

			return false;



		if( $nominacion['autor'] == '' || strlen($nominacion['autor']) > 50 || !preg_match('/^[a-zA-Z0-9ñÑÁáÉéÍíÓóÚúÀàÈèÌìÒòÙùÄäËëÏïÖöÜüÂâÊêÎîÔôÛûÇçß_\-.;\'\s]+$/', $nominacion['autor']) )
		...
		if($nominacion['propuestoPara'] != 'a«Ğadir' && $nominacion['propuestoPara'] != 'eliminar')
	...
	function validarCamposClubLectura($clubLectura){
		...
		if( $clubLectura['nombre'] == '' || strlen($clubLectura['nombre']) > 20 || !preg_match('/^[a-zA-Z0-9ñÑÁáÉéÍíÓóÚúÀàÈèÌìÒòÙùÄäËëÏïÖöÜüÂâÊêÎîÔôÛûÇçß\s]+$/', $clubLectura['nombre']) || existeRegistro('nombre', $clubLectura['nombre'], 'clubLectura') )
