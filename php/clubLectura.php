	...
	if( $obj['opcion'] == 'clubLectura' && $obj['accion'] == 'abrir' ){
		...
		if( $clubLecturaValidado ){
			...
			for( $i=0; $i < count($obj['miembrosClub']); $i++ ){
				...
				$condicion = 'id_club = '.$clubLectura->obtenerId().' AND id_usuario = '.$obj['miembrosClub'][$i]['id_usuario'];
				if(!existeCondicion('miembro_club', $condicion )){
			...
			$campos = 'id_club, creado_por, nombre, f_inicio, f_fin, id_titulacion, curso';
			$tabla  = 'club_lectura';
			$where  = 'f_fin is null and id_club = '.$clubLectura->obtenerId();
			$respuesta['club'] = consulta( $campos, $tabla, $where );
